...
import { Calendar, Map, ArrowRight, FileText, Plus } from 'lucide-react';
...
const DocumentDashboard = () => {
        ...
        try {
            ...
            if (response.data.trips) {
                ...
                if (response.data.trips.length === 1 && !urlTripId) {
                    ...
                    navigate(`/documents/${response.data.trips[0].id}`, { replace: true });
                }
            }
        } catch (error) {
            console.error('Error fetching trips:', error);
            toast.error(t('errors.failedFetch', 'Failed to fetch trips'));
        } finally {
            setLoading(false);
        }
    };
            ...
            <div className="h-full flex items-center justify-center p-4 page-transition">
                <div className="text-center max-w-md">
                    ...
                    <p className="text-gray-500 dark:text-gray-400 mb-6">
                        {t('documents.noTripsMessage', 'Create a trip first to start managing documents')}
                    </p>
                    <Link to="/trips/new">
                        ...
                        <div className="flex items-center gap-3 mb-2">
                            ...
                            <div>
                                <h1 className="text-2xl font-display font-semibold text-gray-900 dark:text-white">
                                    {t('documents.selectTrip', 'Select a trip')}
                                </h1>
                                <p className="text-gray-500 dark:text-gray-400">
                                    {t('documents.selectTripDescription', 'Choose which trip to manage documents for')}
                                </p>
                            </div>
                        </div>
                    </div>
                            ...
                            >
                                ...
                                <div className="p-4 flex items-center justify-between">
                                    ...
                                    <span className="text-accent font-medium text-sm flex items-center group-hover:underline">
                                        {t('documents.view', 'View documents')}
    ...
    // Main documents view
    if (selectedTrip) {
        return <DocumentsList tripId={selectedTrip.id} trip={selectedTrip} />;
    }
